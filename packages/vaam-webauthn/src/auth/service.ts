...
class WebAuthnAuthService extends AbstractAuthModuleProvider {
	...
	async authenticate(
		...
		const success = await this.api.verifyAuthenticationResponse({
			username,
			authIdentity,
		...
		const copy = this.cleanCopy(authIdentity);
	...
	async register(
		...
		try {
			...
			if (error.type === MedusaError.Types.NOT_FOUND) {
				...
				const copy = this.cleanCopy(createdAuthIdentity);
	...
	protected cleanCopy(authIdentity: AuthIdentityDTO) {
		const copy = JSON.parse(JSON.stringify(authIdentity)) as AuthIdentityDTO;
